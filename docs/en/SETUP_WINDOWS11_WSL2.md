...
For **native Windows builds** (declarative deps via vcpkg + CMake presets, without the historical DLL download scripts), see [NATIVE_BUILD_VCPKG.md](NATIVE_BUILD_VCPKG.md). After a Release build, **`tools/windows/Run-WindowsNativeTestSuite.ps1 -WithSmoke`** runs the same Python checks as Linux CI plus native smoke + `baseline_manifest_ci.json` compare (no Docker). **PowerShell** checks for `download_*.ps1` (parse / integration) and saving logs under **`log/`**: [WINDOWS_VERIFICATION.md](WINDOWS_VERIFICATION.md). The flow below is the **Docker** path (same environment as CI smoke).
